...
- `src/eval/eval_core.*`: mixed infrastructure. Generic expression/type/value/cache shapes are reusable in principle. Module arrays and instruction-pointer fields remain as scaffolding, but thread register/process spaces, thread architecture/unwind state, TLS conversion, and register maps are gone.
