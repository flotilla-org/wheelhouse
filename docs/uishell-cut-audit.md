...
Eleventh slice: register/module/TLS evaluation context has been removed. `E_BaseCtx` no longer carries thread register/process spaces, thread architecture/unwind state, or a TLS conversion hook; `E_IRCtx` no longer carries a register-name map; `E_InterpretCtx` no longer carries register space/architecture, module base, or TLS base. Register identifier resolution and register-code coloring are gone. The interpreter still decodes the old `RDI_EvalOp` bytecode format, but register, frame-base, module, and TLS opcodes now fail explicitly instead of consulting debugger state. `MemRead` remains active for config, file, and hash-store spaces until the shell provider model replaces that path.
...
- `src/eval/eval_ir.*`: heavily mixed. It builds RDI-flavored IR/op lists for memory/type evaluation. Register identifier resolution is gone. Shell provider lookup should bypass or replace most of this path rather than pretending app-state rows are debuggee expressions.
- `src/eval/eval_interpret.*`: mostly debugger/RDI VM. It decodes `RDI_EvalOp` and still supports memory reads through the selected shell eval space, but register, frame-base, module, and TLS opcodes fail explicitly. Debug constant data and RDI frame-base setup are gone.
