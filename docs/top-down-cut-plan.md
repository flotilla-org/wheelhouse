...
Command queue storage is the first stable owner moved onto that shell packet. `RD_Cmd.regs` now stores `UIShell_Regs *` directly. The common `rd_cmd_name(...)` emission macro builds a `UIShell_Regs` literal and pushes it through `rd_push_stored_cmd`, so named command emissions no longer construct a temporary `RD_Regs` packet. Autocompletion register emissions use the same shell packet path. Direct shell command emissions that used to pass the live register stack through `rd_push_cmd(name, rd_regs())` now use `rd_push_cmd_current(name)`, which snapshots current shell regs directly. Replaying or deferring an already queued command uses `rd_push_stored_cmd`, preserving the shell-owned packet without reinterpreting it as a debugger packet. Popup-confirmed commands are captured directly as `UIShell_Regs`, so they no longer allocate an intermediate `RD_Regs` snapshot before entering popup command storage. Top-level shell command dispatch copies the queued `UIShell_Regs` into the live shell register stack for the duration of dispatch. A preprocessed shell audit confirms the command list stores `UIShell_Regs *`, the insertion path uses `uishell_regs_copy`, and command emission no longer has an RD-to-shell queue adapter.
