...
The shell dispatch helper now owns query lifecycle execution as well: push query, complete query, cancel query, and update query dispatch by command name in `src/uishell/uishell_dispatch.h`. The behavior is intentionally still equivalent to RAD's lister/query mechanics, but `UI_SHELL_APP` no longer compiles those RAD enum switch cases. Query lifecycle emissions now use `uishell_cmd(...)`, so these commands no longer need generated RAD command enum mapping.
...
The shell target now owns active register slot metadata as well. `UIShell_AppRegSlot` is the single app/query slot enum at the shared command boundary, and `src/uishell/uishell_meta.h` provides `uishell_app_reg_slot_info_table` for the shell-relevant register subset. The live register packet uses `UIShell_ContextRegSlot` to identify slots in `UIShell_Regs`, with explicit conversion from app/query slots at query-completion time. `RD_Regs`, `RD_AppRegSlot`, and `RD_RegSlot` have now been removed from source, along with the old generated RAD register slot name/range arrays.

The shell target now owns command/query metadata types. `src/uishell/uishell_commands.h` defines `UIShell_Query` and `UIShell_CmdInfo`, so shell command definitions no longer use generated `RD_Query` or `RD_CmdKindInfo`. Shared UI paths that only need app command metadata now read through `UIShell_AppCmdInfo`, which adapts to shell metadata. The generated `RD_CmdKindInfo` type/table has been removed from the shell tree; active shell command listing, command-palette/open-tab execution, shell query setup/completion, menu construction, and schema expand-command search now use shell command rows directly.

The shell command/query metadata also now owns its identifiers. Shell command rows use `UIShell_CmdFlag_*`, `UIShell_QueryFlag_*`, and `UIShell_RegSlot_*` instead of spelling RAD command flags, query flags, or generated register slots directly. Shared command metadata stores shell flag masks directly, so the old flag adapter helpers are gone. The only remaining command-query slot conversion maps the smaller `UIShell_RegSlot` query-slot enum into `UIShell_AppRegSlot` through `uishell_app_reg_slot_from_query_reg_slot`.

`UIShell_AppCmdInfo` no longer exposes RAD-owned register slots in its query metadata. It now carries `UIShell_AppRegSlot`. Shell command metadata maps from the smaller `UIShell_RegSlot` query-slot enum into that app slot type, and command query completion converts that into a `UIShell_ContextRegSlot` at the `uishell_regs_fill_slot_from_string` boundary.

The shell target also no longer uses or generates the old all-fields register initializer. `src/raddbg/raddbg_core.h` defines `UISHELL_REGS_LIT_INIT_TOP` directly and copies only the shell UI/config/file/query fields by default. Ordinary shell `UIShell_RegsScope` and `uishell_push_regs` no longer implicitly preserve debugger execution fields such as machine, process, thread, debug-info keys, virtual offset ranges, or disassembly preferences. Named command and autocompletion emissions use the same shell-owned packet.
...
The executable/object-format readers are no longer shell surface. `coff`, `pe`, `elf`, `macho`, and `gnu` have left the unity build and their source directories have been deleted, along with the temporary DW/EH compatibility shim that existed only for those readers. The shell binary view remains a raw byte/hex file viewer. Direct architecture-register metadata has also left the shell unity: `src/uishell/uishell_main.c` no longer includes `x64/x64.*` or `arm64/arm64.*`, so the guarded x64/ARM64 register tables in `arch` and `rdi_local` are not compiled for `uishell`. The shell unity also no longer includes `rdi/rdi_local.*` or `rdi_parse`; it includes `lib_rdi/rdi.*` directly. `lib_rdi` still remains because the evaluator still uses the RDI eval opcode format as its internal bytecode representation.
...
Shell register snapshots are now also copied through a shell-sized boundary. `src/raddbg/raddbg_core.h` defines `UIShell_Regs`, a shell-owned register packet with only UI/config/file/query fields. Under `UI_SHELL_APP`, `uishell_regs`, `uishell_base_regs`, `uishell_regs_copy`, and `uishell_regs_copy_contents` own live access and arena-owned string/list copying for stored snapshots and pushed register scopes, and the live stack node is `UIShell_RegsNode`; there is no longer a `RD_Regs` type/layout, RD-named accessor, or RD-named conversion helper. Query-slot filling for shell builds no longer compiles machine/process/module/thread/control-entity parsers, debugger-only numeric slots, or thread-register promotion. The generated-slot/app-slot adapter is shell-sized under `UI_SHELL_APP`, so debugger register slots map to `Null` instead of remaining active shell metadata.

Command queue storage is the first stable owner moved onto that shell packet. `UIShell_Cmd.regs` now stores `UIShell_Regs *` directly. The common `uishell_cmd(...)` emission macro builds a `UIShell_Regs` literal and pushes it through `uishell_push_stored_cmd`, so named command emissions no longer construct a temporary `RD_Regs` packet. Autocompletion register emissions use the same shell packet path. Direct shell command emissions that used to pass the live register stack through `rd_push_cmd(name, rd_regs())` now use `uishell_push_cmd_current(name)`, which snapshots current shell regs directly. Replaying or deferring an already queued command uses `uishell_push_stored_cmd`, preserving the shell-owned packet without reinterpreting it as a debugger packet. Popup-confirmed commands are captured directly as `UIShell_Regs`, so they no longer allocate an intermediate `RD_Regs` snapshot before entering popup command storage. Top-level shell command dispatch copies the queued `UIShell_Regs` into the live shell register stack for the duration of dispatch. A preprocessed shell audit confirms the command list stores `UIShell_Regs *`, the insertion path uses `uishell_regs_copy`, and command emission no longer has an RD-to-shell queue adapter.
...
1. **Continue shell-neutralizing query/config scaffolding.** Shell config registration, active slot table lookups, active command/query metadata, binding remaps, and default register-scope copying now use shell-owned boundaries. The remaining register compatibility name is explicit code now, not generated metadata, and the debugger-only tail has been removed.
2. **Fork or shrink the register/query layer.** The live register packet and access/push/pop/scope APIs now use shell-owned names. Continue replacing stable shell-facing metadata names with shell-owned equivalents, keeping window, panel, tab, view, file path, cursor/mark, text key, UI key, expression, string, and window-manager event data.
...
5. Continue replacing inherited register-slot metadata names with shell-owned equivalents, after confirming query completion and slot filling stay stable.
