...
The shell target now owns active register slot metadata as well. `UIShell_AppRegSlot` is the single app/query slot enum at the shared command boundary, and `src/uishell/uishell_meta.h` provides `uishell_app_reg_slot_info_table` for the shell-relevant register subset. Shared register parsing/fill helpers now get slot names through shell metadata, and the old generated RAD register slot name/range arrays have been removed from this tree. The shell path adapts from the temporary `RD_RegSlot` compatibility enum to shell metadata in one explicit switch instead of indexing shell tables by `RD_RegSlot_COUNT`. `RD_Regs` and `RD_AppRegSlot` have now been removed from source; the remaining register compatibility surface is the live/context slot enum and slot-fill helper naming.
...
`RD_AppCmdInfo` no longer exposes RAD-owned register slots in its query metadata. It now carries `UIShell_AppRegSlot`. Shell command metadata maps from the smaller `UIShell_RegSlot` query-slot enum into that app slot type, and the only conversion back to `RD_RegSlot` for command query completion is now the explicit `rd_reg_slot_from_app_reg_slot` call at the `rd_regs_fill_slot_from_string` boundary.
