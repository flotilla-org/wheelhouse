...
2. **Fork or shrink the register/query layer.** `RD_Regs` still exists because live register scopes, widgets, and drag/drop state expect one packet. Continue replacing stable shell-facing storage sites with `UIShell_Regs`, keeping window, panel, tab, view, file path, cursor/mark, text key, UI key, expression, string, and window-manager event data.
