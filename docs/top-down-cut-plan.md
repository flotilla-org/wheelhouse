...
`RD_AppCmdInfo` no longer exposes RAD-owned register slots in its query metadata. It now carries `UIShell_AppRegSlot`. Shell command metadata maps from the smaller `UIShell_RegSlot` query-slot enum into that app slot type, and command query completion converts that into a `UIShell_ContextRegSlot` at the `uishell_regs_fill_slot_from_string` boundary.
