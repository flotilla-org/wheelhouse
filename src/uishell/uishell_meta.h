...
read_only global UIShell_NameSchemaInfo uishell_name_schema_info_table[] =
{
  {str8_lit_comp("user"), 0, str8_lit_comp(
    ...
    "@default(0) @display_name('Tabs In Title Bar') @description(\"Renders the main workspace's top-row panel tabs in the window title bar. Requires the native menu bar (to free the title-bar row).\") @expand_if(\"$.mac_native_menu_bar\") 'tabs_in_title_bar': bool,"
