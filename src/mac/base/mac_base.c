...
#if !defined(BUILD_IPC_NAME_PREFIX)
# define BUILD_IPC_NAME_PREFIX "app"
#endif
...
mac_posix_ipc_name(Arena *arena, String8 name)
{
  ...
  String8 result = str8f(arena, "/" BUILD_IPC_NAME_PREFIX "_%llx", hash);
  return result;
}
...
semaphore_alloc(U32 initial_count, U32 max_count, String8 name)
{
  ...
  {
    name_to_open = str8f(scratch.arena, "/" BUILD_IPC_NAME_PREFIX "-%u-%llu", getpid(), ins_atomic_u64_inc_eval(&mac_state.default_env_count));
    generated_name = 1;
  }
